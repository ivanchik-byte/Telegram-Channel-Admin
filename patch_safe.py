...
# 2. Update cb_edit_post
idx1 = handlers.find('@router.callback_query(F.data.startswith("edit_")')
idx2 = handlers.find('@router.message(Command("start"))', idx1)
...
"""
    handlers = handlers[:idx1] + edit_replacement + handlers[idx2:]

# 3. Update process_change_media
media_target = "    await state.set_state(MediaReplacement.waiting_for_media)"
media_replacement = "    await state.update_data(post_id=post_id)\n    await callback.message.delete()\n    await callback.message.answer(f'Пришлите новое медиа (фото/видео/файл) для поста {post_id}:')\n    await state.set_state(MediaReplacement.waiting_for_media)"
handlers = handlers.replace(media_target, media_replacement)

# 4. Update receive_new_media
idx3 = handlers.find('@router.message(MediaReplacement.waiting_for_media, IsModeratorFilter())')
idx4 = handlers.find('@router.message(Command(\'parse\'), IsModeratorFilter())', idx3)
...
"""
    # Look back a bit to not delete "# --- Reply Keyboard Button Handlers ---"
    # Actually idx4 is `@router.message(Command('parse')`
    # We should just find the start of the next handler or the `# --- Reply...`
    idx4_real = handlers.rfind('# --- Reply Keyboard Button Handlers ---', idx3, idx4)
    if idx4_real != -1:
        handlers = handlers[:idx3] + receive_media_replacement + handlers[idx4_real:]
    else:
        handlers = handlers[:idx3] + receive_media_replacement + handlers[idx4:]
...
idx_best_start = tasks.find('prompt = "Ниже список постов. Выбери ОДИН самый интересный')
idx_best_end = tasks.find('    # Process best_id', idx_best_start)

if idx_best_start != -1 and idx_best_end != -1:
...
"""
    tasks = tasks[:idx_best_start] + best_replacement + tasks[idx_best_end:]

# Also replace the processing part
idx_process_start = tasks.find('    # Process best_id')
idx_process_end = tasks.find('        else:\n            logger.error(f"[Worker] Выбранный ID {best_id} не найден в списке!")', idx_process_start) + len('        else:\n            logger.error(f"[Worker] Выбранный ID {best_id} не найден в списке!")')

if idx_process_start != -1 and idx_process_end != -1:
    process_replacement = """    async with async_session_maker() as session:
        found_first = None
        for p in posts:
            if p.id in best_ids:
    ...
    tasks = tasks[:idx_process_start] + process_replacement + tasks[idx_process_end:]
