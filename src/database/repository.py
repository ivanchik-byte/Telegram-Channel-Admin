...
class PostRepository:
    @staticmethod
    async def process_new_post(session: AsyncSession, channel_id: int, message_id: int, post_hash: str, text: str):
        """
        ...
        try:
            ...
            logger.info(f"Duplicate post detected, ignoring: {e}")
            return None
    ...
    @staticmethod
    async def update_rewritten_text(session: AsyncSession, post_id: int, new_text: str):
        from sqlalchemy import update
        stmt = update(ProcessedPost).where(ProcessedPost.id == post_id).values(rewritten_text=new_text)
        await session.execute(stmt)
        await session.commit()
