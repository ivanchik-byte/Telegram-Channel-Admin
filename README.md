...
* `/best` — принудительно запустить парсер и выбрать ТОП-6 лучших постов за последние 24 часа (1 сразу на ревью, 5 в очередь) с автоматическим сбросом интервала.
* `/best 24h` (или `30m`, `2d`) — отбор лучших постов за указанный период.
* `/parse` — принудительно запустить парсер для загрузки последних 5 сообщений из всех каналов.
* `/parse [limit] [num_channels]` — запустить парсер с указанием количества сообщений и случайных каналов (например, `/parse 5 3` для сбора по 5 постов с 3 случайных каналов).
...
* **Работа без группы**: Вы можете модерировать посты прямо в личных сообщениях с ботом, оставив параметр `MODERATOR_CHAT_ID=""` в `.env`. Карточки будут приходить первому администратору из списка `ADMIN_IDS`.
* **Журнал действий**: При публикации или отклонении поста бот автоматически добавляет подпись вида `👤 Действие от: @юзернейм` к карточке. Это позволяет команде модераторов (в случае использования общей группы) видеть, кто принял решение по конкретному посту.
