...
* `/status` — посмотреть текущую загруженность очереди, выбранный режим (интервалы выводятся в секундах) и время до снятия паузы.
* `/clear` — удалить все посты, ожидающие модерации, и очистить кураторскую корзину.

> [!TIP]
> Команды времени поддерживают суффиксы: `s` (секунды), `m` (минуты), `h` (часы), `d` (дни). Если суффикс не указан, по умолчанию используются **секунды**. Например, `/pause 30` и `/pause 30s` — это одно и то же.
