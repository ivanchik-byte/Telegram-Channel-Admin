...
flowchart TD
    ...
    Parser -->|Добавление задачи на обработку| Queue["Очередь задач (Redis + Arq)"]
    Queue -->|Получение задачи| Worker["Воркер (API ИИ)"]
    Worker -->|Если найдена реклама: статус filtered_ad| DB
    Worker -->|Если рекламы нет: сохранение рерайта| DB
    Worker -->|Сигнал о готовности| Bot["Модераторский бот (aiogram)"]
    Bot -->|Карточка на ревью| ModGroup["Чат модераторов"]
    ModGroup -->|Решение: одобрить, отклонить, изменить| Bot
    Bot -->|Обновление статуса поста| DB
    Bot -->|Публикация одобренного поста| TargetChannel["Целевой канал"]
```
